...
// 공개 화면 공용 요청 헬퍼(lib/fetch-state.ts): 통신 실패(network/5xx/429)와 "결과 없음"이 절대 섞이지 않는지 고정한다.
import assert from "node:assert/strict";
import { request, errText, newIdemKey } from "../lib/fetch-state.ts";
...
assert.equal(errText(await request("/x"), t), "common.genericError");
...
console.log("fetch-state: all assertions passed");
