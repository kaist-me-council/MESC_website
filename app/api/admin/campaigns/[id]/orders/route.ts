...
import { ORDER_STATUSES, type OrderItem, type Resolution } from "@/lib/campaign";
...
export async function GET(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const orders = rows.map(({ studentIdHash, items, resolution, ...o }) => ({
    ...o,
  ...
  if (new URL(req.url).searchParams.get("format") === "csv") {
    ...
    const head = ["주문번호", "상태", "구분", "이름", "이메일", "전화", "항목", "합계", "메모", "관리자메모", "신청시각", "출처", "수령확인", "처리선택", "확인메모", "확인시각"];
    const lines = orders.map((o) =>
      [o.orderNo, STATUS_KO[o.status] ?? o.status, o.affiliation, o.name, o.email, o.phone, itemStr(o.items), o.total, o.note, o.adminMemo, new Date(o.createdAt).toLocaleString("ko-KR"),
...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (Array.isArray(b.orderIds)) {
    ...
    const { count } = await prisma.campaignOrder.updateMany({ where: { id: { in: ids }, campaignId: id }, data: { status: String(b.status) } });
  ...
  const data: { status?: string; adminMemo?: string | null; confirmation?: string | null; confirmedAt?: Date | null; resolution?: null } = {};
