...
export async function PUT(req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (code === "P2025") return NextResponse.json({ error: "전공서를 찾을 수 없습니다." }, { status: 404 });
...
export async function DELETE(_req: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    if (getPrismaCode(error) === "P2025") return NextResponse.json({ error: "전공서를 찾을 수 없습니다." }, { status: 404 });
    return NextResponse.json({ error: "전공서 삭제 중 오류가 발생했습니다." }, { status: 500 });
  }
}
