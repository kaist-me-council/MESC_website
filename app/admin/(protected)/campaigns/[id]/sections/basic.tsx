...
export function BasicSection({ c, set, setC }: SectionProps) {
  ...
  return <div className="grid gap-4 sm:grid-cols-2">
    ...
    <div className="flex flex-wrap gap-5 sm:col-span-2"><Toggle checked={c.enabled} onChange={(v) => set("enabled", v)} label="공개" /><Toggle checked={c.allowQty} onChange={(v) => set("allowQty", v)} label="수량 선택" /><Toggle checked={c.requireStudentId} onChange={(v) => set("requireStudentId", v)} label="학번 필수" help={c.requireStudentId ? undefined : "학생 화면에 선택으로 표시"} /><Toggle checked={c.requirePhone} onChange={(v) => set("requirePhone", v)} label="전화번호 필수" help={c.requirePhone ? undefined : "학생 화면에 선택으로 표시"} /></div>
